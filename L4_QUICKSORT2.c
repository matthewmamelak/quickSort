// QUICKSORT [161114]
// LAB4_QUICKSORT2.C - Lab 04 - Cramarossa, Alexandra and Mamelak, Matthew
...
//Here the function will work through an array of numbers and place each one in the correct place.
//With insertion sort, one value is moved to its relative place.
{
    int     x, y;
    int     result = -1;
    ...
    if (a == NULL || (min > max))   {
        ...
        //To know how many exchanges take place we initialize swaps

        //For loop works through the values from min to max
        //Start at position 0 then add the rest of the array one by one to the sorted part
        for(x = min; x <=max; x++){
            //Now i shows which new element in the array will be inserted into the correct position
            //At the end of each loop, one value is moved and then everything at the bottom is ordered
            for(y = x; y> min && (a[y - 1] > a[y]); y--){
